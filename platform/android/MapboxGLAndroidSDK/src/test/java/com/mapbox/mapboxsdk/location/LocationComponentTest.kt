...
class LocationComponentTest {
  ...
  fun transitionCallbackFinishedTest() {
    ...
    verify(locationCameraController).setCameraMode(eq(CameraMode.TRACKING), any(), callback.capture(), eq(TRANSITION_ANIMATION_DURATION_MS), isNull(), isNull(), isNull())
  ...
  fun transitionCustomFinishedTest() {
    ...
    val callback = ArgumentCaptor.forClass(OnLocationCameraTransitionListener::class.java)
    locationComponent.setCameraMode(CameraMode.TRACKING, listener, 1200, 14.0, 13.0, 45.0)
    verify(locationCameraController).setCameraMode(eq(CameraMode.TRACKING), any(), callback.capture(), eq(1200L), eq(14.0), eq(13.0), eq(45.0))
    callback.value.onLocationCameraTransitionFinished(CameraMode.TRACKING)
